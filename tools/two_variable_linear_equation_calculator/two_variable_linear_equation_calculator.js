...
router.get('/:language/tools/two_variable_linear_equation_calculator', checkPageRedirect, (req, res) => {
    fs.readFile(path.join(config.path, "tools", "tools.json"), 'utf8', (err, data) => {
        if (data) {
            const Tools = JSON.parse(data);
            let thisTool;
            for (let i = 0; i < Tools.length; i++) {
                if (Tools[i].url == "two_variable_linear_equation_calculator") thisTool = Tools[i];
            }
...
router.post('/:language/tools/two_variable_linear_equation_calculator', checkPageRedirect, (req, res) => {
