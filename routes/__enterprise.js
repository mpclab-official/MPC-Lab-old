// __enterprise.js
// This is a router file that contains the code for the enterprise page.
...
router.get(
  ...
  (req, res) => {
    getUserData(req.session.userID, (err, data) => {
      ...
      Enterprise.getEnterpriseById(req.params.enterpriseID).then(
        (enterprise) => {
          if (!enterprise) {
            res.status(404).redirect(`/${req.params.language}/404`);
            return;
          }
          res.render(path.join(config.path, "page", "enterprise"), {
            title: `${config.name} | ${enterprise.name}`,
