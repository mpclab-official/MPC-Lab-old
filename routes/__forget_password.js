// __forget_password.js
// This is a router file that contains the code for the forget password page.
...
router
  .route("/:language/forget-password")
  .get(checkPageRedirect, (req, res) => {
    getUserData(req.session.userID, (err, data) => {
      ...
      res.render(path.join(config.path, "page", "forget-password"), {
        title: `${config.name} | ${languages_translate_pack.lan_auth_forget_password}`,
