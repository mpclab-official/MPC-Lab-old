    ...
    .post((req, res) => {
        ...
        if (params.UserData) {
            ...
            if (UserData.userAvatar) {
                // Extract MIME type
                const mimeInfo = UserData.userAvatar.match(/^data:(image\/\w+);base64,/);

                // Base 64 data
                const mimeType = mimeInfo[1];
                const base64Data = UserData.userAvatar.replace(/^data:image\/\w+;base64,/, '');
                const buffer = Buffer.from(base64Data, 'base64');

                // Check if the folder exists, if not, create it
                const avatarFolderPath = path.join(config.path, 'public', 'users-avatar');
