    ...
    fetch(`/${languageCode}/login`, {
        ...
        .then(json => {
            if (json.code.includes(0)) {
                ...
                markInvalidInput(document.getElementById('loginForm').elements['username'], json.messages[1]);
                markInvalidInput(document.getElementById('loginForm').elements['password'], json.messages[1]);
            } else if (json.code.includes(2)) {
                markInvalidInput(document.getElementById('loginForm').elements['username'], json.messages[2]);
            }
        })
        .catch(error => {
...
function submitResetPasswordForm() {
    ...
    const email = document.getElementById('forget_passwordForm').elements['email'].value;
    const verification_code = document.getElementById('forget_passwordForm').elements['verification_code'].value;
    const password = document.getElementById('forget_passwordForm').elements['password'].value;
...
function submitVerificationCode() {
    ...
    if (email) {
        fetch(`/${languageCode}/verification`, {
            ...
            .then(json => {
                if (json.code.includes(0)) {
                    ...
                    markInvalidInput(form.elements['verification_code'], json.messages[1]);
                }
            })
            .catch(error => {
                console.error('Error:', error.message);
        ...
        var repeat = 100;
        var text = document.getElementsByClassName("get-verification-code")[0].innerHTML;
        var timer = setInterval(function () {
            if (repeat == 0) {
                document.getElementsByClassName("get-verification-code")[0].innerHTML = text;
                document.getElementsByClassName("get-verification-code")[0].onclick = submitVerificationCode;
                clearInterval(timer);
            } else {
                repeat--;
                document.getElementsByClassName("get-verification-code")[0].innerHTML = repeat;
                document.getElementsByClassName("get-verification-code")[0].onclick = null;
            }
        }, 1000);
    } else {
        markInvalidInput(form.elements['email'], form.elements['email'].dataset.lan_auth_register_empty_email);
    }
}
